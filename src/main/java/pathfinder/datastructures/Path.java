...
public class Path<E> implements Iterable<Path<E>.Segment> {
  ...
  // AF(this) =
  //      first point(generic) in the path => start
  ...
  /**
   * A generic type representing the beginning of this path.
   */
  private E start;

  /**
   * The ordered sequence of segments representing a path between generic types of points.
   */
  private List<Segment> path;

  /**
   * Creates a new, empty path containing a generic type start point. Essentially this represents a path
   * from the start point to itself with a total cost of "0".
   *
   * @param start The starting point (generic) of the path.
   */
  public Path(E start) {
    this.start = start;
    this.cost = 0;
  ...
  /**
   * Appends a new single segment to the end of this path, originating at the current last generic type point
   ...
   * segmentCost}.
   *
   * @param newEnd      The point (generic) being added at the end of the segment being appended to this path
  ...
  /**
   * @return The point(generic) at the beginning of this path.
   */
  public E getStart() {
    return start;
  }
  ...
  /**
   * @return The point(generic) at the end of this path, which may be the start point(generic) if this path
   * contains no segments (i.e. this path is from the start point(generic) to itself).
   */
  public E getEnd() {
    if(path.size() == 0) {
      return start;
   ...
   * point(generic) and ending at the end point(generic). In the case that this path represents a path between
   * the start point(generic) and itself, this iterator contains no elements. This iterator does not
  ...
  private void checkRep() {
    ...
    assert start != null;
    assert path != null;
    for(Path<E>.Segment segment : path) {
      assert segment != null;
    }
  }
   ...
   * both paths are empty, they are only equal if their starting point(generic) is equal.
   *
  ...
  public class Segment {
     ...
     *
     * @param start The starting point(generic) of this segment.
     * @param end   The ending point(generic) of this segment.
     * @param cost  The cost of travelling this segment.
     * @throws NullPointerException     if either point(generic) is null.
    ...
    /**
     * @return The beginning point(generic) of this segment.
     */
    public E getStart() {
    ...
    /**
     * @return The ending point(generic) of this segment.
     */
    public E getEnd() {
      return this.end;
    }
